...
class CMS < Imprenta
    ...
    def initialize
        ...
        @sitio_titulo                    = "GULAG"
        @sitio_descripcion               = "Grupo de Usuarios de GNU/Linux de la Laguna."
        @sitio_url                       = "http://gulag.org.mx"                   # No use diagonal al final
        @autor_por_defecto               = "guivaloz"                              # El autor por defecto para las publicaciones que no lo tengan definido
        @publicaciones_directorios       = %w{ entradas revista contacto }         # Los directorios que contenen las publicaciones
     #~ @publicaciones_etiquetas         = { 'documentacion' => 'Documentación' }  # Si hay que usar una etiqueta diferente para el directorio
        ...
        @menu_principal.agregar('Revista',        'revista/index.html')
        @menu_principal.agregar('Contacto',       'contacto/index.html')
        @menu_principal.agregar_derecha('GitHub', 'https://github.com/gulag')
...
FINAL
        @plantilla.pie          = <<FINAL
            <p>
                Copyright (c) 2014 Grupo de Usuarios de GNU/Linux de la Laguna. Se otorga permiso para copiar, distribuir y/o modificar este documento.<br>
